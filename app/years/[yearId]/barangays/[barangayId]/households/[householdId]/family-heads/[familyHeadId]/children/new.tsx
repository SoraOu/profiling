...
export default function NewChildScreen() {
  const { householdId, familyHeadId } = useLocalSearchParams<{
    householdId: string; familyHeadId: string;
  }>();
  ...
  async function save() {
    ...
    try {
      ...
      await db.runAsync(
        `INSERT INTO children (family_head_id, household_id, name, birthday, age_in_days, nutritional_statuses, disability, medical_condition)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [familyHeadId, householdId, name, birthday || null, ageInDays,
          JSON.stringify(nutritionalStatuses), disability || null, medicalCondition || null]
      );
      router.back();
    } catch (e) {
  ...
  return (
    <>
      <Stack.Screen options={{ title: "New Child" }} />
      <ScrollView style={styles.container} contentContainerStyle={styles.content}>
        <Text style={styles.sectionTitle}>Chapter VI — Child Health & Nutrition</Text>
        ...
        <TouchableOpacity style={styles.saveBtn} onPress={save} disabled={saving}>
          <Text style={styles.saveBtnText}>{saving ? "Saving..." : "Save Child"}</Text>
        </TouchableOpacity>
      </ScrollView>
    </>
  );
